...
def view_winning_draw():
    ...
    decrypted_current_winning_draw = copy.deepcopy(current_winning_draw)

    # decrypt copy of post object.
    decrypted_current_winning_draw.view_draw(current_user.draw_key)

    # if a winning draw exists
    if current_winning_draw:
        # re-render admin page with current winning draw and lottery round
        return render_template('admin.html', winning_draw=decrypted_current_winning_draw, name=current_user.firstname)
    ...
    if current_winning_draw:
        ...
        if user_draws:
            ...
            for draw in user_draws:
                ...
                # decrypt the specific draw with the draw key of the user who made it
                draw_values = decrypt(draw.draw, user.draw_key)

                # decrypt the current winning draw
                winning_draw_values = decrypt(current_winning_draw.draw, current_user.draw_key)

                # if user draw matches current unplayed winning draw
                if draw_values == winning_draw_values:
                    # add details of winner to list of results
                    results.append((current_winning_draw.round, draw_values, draw.user_id, user.email))
...
@requires_roles('admin')
def logs():
    # open and get the last 10 lines of the log file
    with open("lottery.log", "r") as f:
        content = f.read().splitlines()[-10:]
        content.reverse()

    # pass these lines into the template so we can show them
